...
class Solution{
    ...
    vector<int>check(Node *root,int &ans){
        ...
        if(!root) return {INT_MAX,INT_MIN,0};// min,max,size
    ...
    // then this tree is not a BST.
        ...
        int min_ele=min({lft[0],rgt[0],root->data});
        int max_ele=max({lft[1],rgt[1],root->data});
        ans=max(ans,size);
    ...
    int largestBst(Node *root)
    {
    	//Your code here
    	if(!root) return 0;
    	int ans=1; // min ans will always be 1
    	check(root,ans);
    	return ans;
    }
};
